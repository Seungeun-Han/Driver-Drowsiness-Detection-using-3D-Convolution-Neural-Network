...
####################################################################################################################
############################################## 이 부분 필요시 수정 ####################################################
mirrored_strategy = tf.distribute.MirroredStrategy(devices=["/GPU:0", "/GPU:1", "/GPU:2", "/GPU:3"])  ### 필요시 수정 ###

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='simple 3D convolution for action recognition')
    parser.add_argument('--batch', type=int, default=128)  ### 필요시 수정 ###
    parser.add_argument('--epoch', type=int, default=100)  ### 필요시 수정 ###
    ...
    parser.add_argument('--fovp', type=str, default='fr016_ovp000')  ### 필요시 수정 ### 세자릿수 맞춰서! 10프레임 5오버랩이면 'fr010_ovp005'
    args = parser.parse_args()

    train_label = np.load('./Labels_f16p0_00044646_mouth.npy')  ### 필요시 수정 ###
    mouth_train_data = np.load("./Inputs_f16p0_00044646_mouth.npy")  ### 필요시 수정 ###
    righteye_train_data = np.load("./Inputs_f16p0_00044646_righteye.npy")  ### 필요시 수정 ###
    # face_train_data = np.load("./Inputs_f16p0_00044646_064_face.npy")
    ...
    m_X_train, m_X_test, m_Y_train, m_Y_test = train_test_split(mouth_train_data, train_label, test_size=0.2,
                                                                random_state=43)
    r_X_train, r_X_test, r_Y_train, r_Y_test = train_test_split(righteye_train_data, train_label, test_size=0.2,
                                                                random_state=43)
